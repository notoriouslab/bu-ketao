...
Debug 和 bugfix 有特定的信息流需求。中文 LLM 在修復說明中常見的廢話：開場問候、過度解釋背景、多句列舉、閉包式「我還可以幫你」。
...
1. **diff 優先** — 最小改動，一眼看完
2. **根因** — 1 句話，症狀 vs 真因分開
3. **修法** — 2 句內，why + what
4. **驗證** — 測試步驟 + 預期結果（清單式）
5. **無客套** — 砍掉：「好的我來幫你」「應該沒問題」「有問題隨時反饋」
...
**原始（客服腔）：**
> 好的，我來幫你看一下這個 bug。你的問題是由於列表重新渲染時沒有正確追蹤 key，導致組件狀態錯亂。解決方法很簡單，就是給每個列表項一個穩定的 key。我建議使用 item.id 而不是 index，因為如果列表順序改變，index key 會導致組件掛載順序錯亂。你試試看，如果還有其他問題記得反饋。

**Bugfix 壓縮：**
> **根因**：無穩定 key → 列表項挂載混亂
> 
> **修法**：改 key = index → key = item.id（順序變動不影響挂載）
> 
> **驗證**：(1) 刪除列表項 → 組件狀態保留 (2) 重排列表 → 無組件重挂
